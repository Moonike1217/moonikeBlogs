...
G1 (Garbage-First) 是一款面向服务器的垃圾收集器，主要针对配备多颗处理器及大容量内存的机器，以极高概率满足 GC 停顿时间要求的同时,还具备高吞吐量性能特征。它开创了 **面向局部的垃圾收集思路** 和 **基于 Region 的内存布局形式** ，并且 G1 收集器建立了一个 **停顿时间模型** 。因此从 JDK 9 开始，G1 收集器成为了默认的垃圾收集器。
